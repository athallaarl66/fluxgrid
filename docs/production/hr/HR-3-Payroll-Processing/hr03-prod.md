...
2. **Data Aggregation**: The system automatically pulls `Base Salary` (from HR-1), `Approved Overtime` & `Late Minutes` (from HR-2).
...
- [ ] Automatic aggregation of attendance data (Lateness penalties, Overtime pay).
...
- Dependent on **HR-1** (Base Salary) and **HR-2** (Attendance).
