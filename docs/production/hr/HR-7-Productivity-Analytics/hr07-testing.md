...
- **Given** Employee A worked 160 hours this month (from Attendance) and logged 150 hours of active task time (from TaskProject).
...
- Analytics dashboards are notorious for slowing down databases. Ensure that the dashboard relies on a pre-aggregated `monthly_productivity_summary` materialized view or summary table, rather than querying raw `attendance_logs` and `tasks` in real-time. The dashboard must load in < 2 seconds.
