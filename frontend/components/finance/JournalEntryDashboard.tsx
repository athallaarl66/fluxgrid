...
import { Plus, RefreshCw, Filter, Eye, Pencil, Trash2, CheckCircle } from "lucide-react";
            ...
            {loading ? (
              ...
              entries.map((entry) => {
                ...
                return (
                  ...
                  >
                    <td className="h-9 px-3 text-sm tabular-nums font-mono text-muted-foreground">
                      {entry.entryNo}
                    </td>
                    <td className="h-9 px-3 text-sm tabular-nums text-foreground">
                      {new Date(entry.transactionDate).toLocaleDateString("id-ID")}
                    </td>
                    <td className="h-9 px-3 text-sm text-foreground truncate max-w-[280px]">
                      {entry.description}
                    </td>
                    <td className="h-9 px-3 text-sm text-right tabular-nums text-foreground font-medium">
                      {formatIDR(entry.totalAmount)}
                    </td>
                    <td className="h-9 px-3">
                      <span className={cn("inline-flex items-center rounded px-1.5 py-0.5 text-[11px] font-semibold", cfg.className)}>
                        {cfg.label}
                      </span>
                    </td>
                    <td className="h-9 px-3">
                      <div className="flex items-center justify-end gap-0.5">
                        <button
                          ...
                          className="flex size-7 items-center justify-center rounded text-muted-foreground transition-colors hover:bg-muted hover:text-foreground opacity-0 group-hover:opacity-100 cursor-pointer"
                          title="View"
                        >
                        ...
                        {entry.status === "PENDING_APPROVAL" && (
                          <button
                            ...
                            className="flex size-7 items-center justify-center rounded text-muted-foreground transition-colors hover:bg-emerald-50 hover:text-emerald-700 opacity-0 group-hover:opacity-100 cursor-pointer disabled:opacity-50"
                            title="Approve"
                          >
                        ...
                        {(entry.status === "DRAFT" || entry.status === "PENDING_APPROVAL") && (
                          <button
                            ...
                            disabled={actionLoading === entry.id + "-void"}
                            className="flex size-7 items-center justify-center rounded text-muted-foreground transition-colors hover:bg-destructive/10 hover:text-destructive opacity-0 group-hover:opacity-100 cursor-pointer disabled:opacity-50"
                            title="Void"
                          >
